...
|		EE			  Ss	MM		 MM		EE																		Methode points milieu				EE			  Ss	MM		 MM		EE
...
#define N 8000000 //définit le nombre de points qui seront pris EN TOUT pour l'évaluation de l'aire. Meilleure précision si N est divisible par le nombre de processus ici : 1, 2, 4, ou 8. Plus N est grand, meilleure est la précision et plus long est le temps de calcul.
...
int main (int argc, char *argv[])
{
	...
	double longueur_tranche, i; //longueur_tranche correspond au sous interval utilisé par chaque processus (exempe : Avec 8 processus, le P0 se charge de x [0;1/8[, et P1 de [1/8;2/8])
	...
	double aire = 0.0, aire_tot = 0.0, fx, x, x2, fx2, dx, fdx; // aire est l'aire calculée par un processus. aire_tot est donc la somme des aires. les autres variables sont plus intuitives d'un ppoint de vu mathématique
	...
	for(i = (rang * longueur_tranche); i < ((rang * longueur_tranche) + longueur_tranche); i += step) //chaque processus parcourt son interval, (cf déclaration de longueur tranche) avec un pas == step
	{
		...
		dx = (x + x2) / 2; //prends le point milieu
		fdx =  (1.0 / ( 1.0 + pow(dx, 2))); //calcule f(dx)
		aire += fdx * step; //calcule l'aire
	}
	...
	if(rang == 0)
	{
		...
		printf("La valeur approchée de Pi est : %.17f \n", aire_tot);//affiche la valeur approximée de Pi, avec
	}
